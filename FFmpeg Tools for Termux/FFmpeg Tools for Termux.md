...
audio_merge() {
    ...
    if [[ -f "$out_path" ]]; then
        ...
        echo -e "  ${DIM}  For mixed formats, re-encode: use Convert feature (coming soon)${RESET}"
    fi
...
After pasting the script, save and exit using these **3 key presses**:
...
> **Tip:** To launch it from anywhere at any time, run:
> ```bash
> mv ffmpeg_tools.sh $PREFIX/bin/ffmpeg_tools
> ```
> Then just type `ffmpeg_tools` in Termux to start it.
...
- **No folder navigation needed** — just type any filename and the script searches your entire device automatically
...
- Run `termux-setup-storage` once if files can't be found (grants storage permission)
