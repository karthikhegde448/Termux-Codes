...
# ---------- Writable temp directory (Termux's /tmp is sandboxed/unwritable) ----------
TMP_DIR="${TMPDIR:-$PREFIX/tmp}"
[[ -z "$TMP_DIR" || ! -d "$TMP_DIR" ]] && TMP_DIR="$HOME/.ffmpeg_tools_tmp"
...
audio_merge() {
    ...
    # Remove any stale leftover file so a failed run can't be mistaken for success
    rm -f "$out_path"

    # Expected total duration = sum of all input durations (for verification)
    ...
    # ---- Attempt 1: fast demuxer concat (lossless, but fragile for mismatched params) ----
    local list_file
    list_file=$(mktemp "${TMP_DIR}/ffmpeg_concat_XXXXXX.txt")
    for f in "${FILE_PATHS[@]}"; do
        echo "file '${f}'" >> "$list_file"
    done

    ffmpeg -f concat -safe 0 -i "$list_file" -c copy "$out_path" -y \
        > "${TMP_DIR}/ffmpeg_merge_log.txt" 2>&1
    local ff_status=$?
    rm -f "$list_file"

    grep -E "error|Error|size=|time=" "${TMP_DIR}/ffmpeg_merge_log.txt" | \
        while IFS= read -r line; do echo -e "  ${DIM}${line}${RESET}"; done

    # Verify: did ffmpeg succeed AND does the duration roughly match the sum of inputs?
    local merge_ok=false
    if [[ $ff_status -eq 0 && -f "$out_path" ]]; then
        local actual_dur
        actual_dur=$(ffprobe -v error -show_entries format=duration \
            -of default=noprint_wrappers=1:nokey=1 "$out_path" 2>/dev/null)
        actual_dur=${actual_dur%.*}
        [[ -z "$actual_dur" ]] && actual_dur=0

        # Allow 2 second tolerance for rounding/container overhead
        local diff=$(( actual_dur - expected_dur ))
        (( diff < 0 )) && diff=$(( -diff ))
    ...
    # ---- Attempt 2: fallback to filter-based concat (re-encodes, but reliable) ----
    if [[ "$merge_ok" == false ]]; then
        echo ""
        echo -e "  ${YELLOW}⚠  Fast merge produced a mismatch — retrying with re-encode...${RESET}"
        echo ""
        ...
        ffmpeg "${input_args[@]}" -filter_complex \
            "${filter_inputs}concat=n=${n}:v=0:a=1[out]" \
            -map "[out]" "$out_path" -y \
            > "${TMP_DIR}/ffmpeg_merge_log.txt" 2>&1
        ff_status=$?
    ...
    rm -f "${TMP_DIR}/ffmpeg_merge_log.txt"
    ...
    if [[ "$merge_ok" == true ]]; then
        ...
        rm -f "$out_path"
        echo ""
        echo -e "  ${RED}✗  Merge failed. Files may be corrupt or in an unsupported format.${RESET}"
        echo -e "  ${DIM}  Tip: try option 5 (Convert Format) on each file first.${RESET}"
    fi
...
video_merge() {
    ...
    local list_file
    list_file=$(mktemp "${TMP_DIR}/ffmpeg_vconcat_XXXXXX.txt")
    for f in "${FILE_PATHS[@]}"; do
        echo "file '${f}'" >> "$list_file"
    done
